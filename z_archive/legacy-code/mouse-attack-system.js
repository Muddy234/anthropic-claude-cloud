...
function performMouseAttack(player, fromBuffer = false) {
    ...
    // STAMINA CHECK: Combo finisher (hit 3) costs stamina
    if (typeof StaminaSystem !== 'undefined') {
        ...
        if (!StaminaSystem.consumeAction(actionType)) {
            // Not enough stamina - show feedback but don't buffer
            // (player should see they're out of stamina, not queue more attacks)
            return false;
        }
    }
...
function applyMeleeDamage(player, enemy, isSpecial) {
    ...
    // IMPACT FRAMES: Trigger hitstop with variable duration based on weapon weight
    triggerHitstop(damageResult.isCrit, isSpecial, {
        isAmbush: damageResult.isAmbush || false,
        isKill: isKill,
        weapon: weapon
    });
    ...
    const effectiveDamage = Math.min(damageResult.finalDamage, hpBeforeDamage);
    ...
    if (enemy.hp > 0) {
        ...
        // Engage combat - FIXED: Must engage BOTH sides
        // engageCombat(attacker, target) sets attacker's target and isInCombat
        if (typeof engageCombat === 'function') {
            engageCombat(player, enemy);  // Player attacks enemy (sets player.combat.currentTarget)
            engageCombat(enemy, player);  // Enemy retaliates (sets enemy.combat.currentTarget)
        }
    }

    // Check for death
    if (enemy.hp <= 0) {
        // STAMINA: Refund stamina on kill
        if (typeof StaminaSystem !== 'undefined') {
            StaminaSystem.onKill();
        }

        // KILL STREAK: Track kill
        if (typeof KillStreakSystem !== 'undefined') {
            KillStreakSystem.onKill(enemy);
        }
 ...
 * Uses HITSTOP_CONFIG for variable duration based on weapon weight
 * @param {boolean} isCrit - Whether this was a critical hit
 * @param {boolean} isSpecial - Whether this was a special attack (combo finisher)
 * @param {Object} options - Additional options { isAmbush, isKill, weapon }
 */
function triggerHitstop(isCrit, isSpecial, options = {}) {
    let duration;

    // Use new HITSTOP_CONFIG if available
    if (typeof HITSTOP_CONFIG !== 'undefined') {
        const { isAmbush = false, isKill = false, weapon = null } = options;

        // Get weapon type from equipped weapon or passed weapon
        const equippedWeapon = weapon || game?.player?.equipped?.MAIN;
        const weaponType = equippedWeapon?.weaponType || 'unarmed';

        // Get weight class from weapon type
        const weightClass = HITSTOP_CONFIG.weaponWeights[weaponType] ||
                           HITSTOP_CONFIG.weaponWeights.default ||
                           'medium';

        // Get base duration from weight class
        duration = HITSTOP_CONFIG.weaponClass[weightClass] || 50;

        // Apply multipliers (stack multiplicatively)
        if (isSpecial) {
            duration *= HITSTOP_CONFIG.multipliers.comboFinisher;
        }
        if (isCrit) {
            duration *= HITSTOP_CONFIG.multipliers.critical;
        }
        if (isAmbush) {
            duration *= HITSTOP_CONFIG.multipliers.ambush;
        }
        if (isKill) {
            duration *= HITSTOP_CONFIG.multipliers.killing;
        }

        // Cap the duration
        duration = Math.min(duration, HITSTOP_CONFIG.maxHitstop);

        // Convert ms to seconds for timer
        duration = duration / 1000;

        // Trigger time slow for heavy hits
        if (duration * 1000 >= HITSTOP_CONFIG.timeSlowThreshold && typeof TimeEffects !== 'undefined') {
            TimeEffects.triggerSlow(HITSTOP_CONFIG.timeSlowDuration, HITSTOP_CONFIG.timeSlowFactor);
        }
    } else {
        // Fallback to old behavior
        duration = isCrit ? mouseAttackState.hitstopCritDuration :
