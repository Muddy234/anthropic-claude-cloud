...
func _ready():
	# Note: Don't connect to turn_ended - GameManager calls process_enemy_turns() directly
	# to avoid race condition with the await for all_enemies_acted signal
...
## Note: This is no longer called during exploration - only during arena combat if needed
...
func _process_next_enemy():
	...
	# Use call_deferred to emit the signal after the current frame
	# This ensures any awaiting code has time to set up the await before the signal fires
	call_deferred("_emit_all_enemies_acted")
...
## Deferred signal emission to avoid race condition with await
func _emit_all_enemies_acted():
	print("DEBUG: Emitting all_enemies_acted signal (deferred)")
	all_enemies_acted.emit()

func _on_player_turn_ended():
	# Note: This is now called directly from GameManager, not via signal
	# to avoid race condition with await. Keeping for backwards compatibility.
	process_enemy_turns()

func _on_floor_changed(_floor_num: int):
	# Note: Don't clear registered_enemies here - floor_entered is emitted AFTER
	# enemies are spawned in dungeon.generate_floor(). Old enemies are automatically
	# unregistered when they die or are queue_free'd. Just clear the pathfinding cache.
	pathfinding_cache.clear()
