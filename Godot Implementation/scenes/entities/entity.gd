...
@export var grid_pos: Vector2i:
	set(value):
		...
		grid_pos = value
		# Center entity on the tile (add half tile size to position)
		_target_position = Vector2(value) * Constants.TILE_SIZE + Vector2(Constants.TILE_SIZE / 2.0, Constants.TILE_SIZE / 2.0)
...
func _ready():
	current_hp = max_hp
	# Center entity on the tile (add half tile size to position)
	position = Vector2(grid_pos) * Constants.TILE_SIZE + Vector2(Constants.TILE_SIZE / 2.0, Constants.TILE_SIZE / 2.0)
	_target_position = position
	add_to_group("entities")

func _process(delta: float):
	# Smooth movement interpolation (constant speed regardless of direction)
	# MOVE_SPEED is tiles per second, multiply by TILE_SIZE to get pixels per second
	if position.distance_to(_target_position) > 1:
		var direction := (_target_position - position).normalized()
		var pixels_per_second := Constants.MOVE_SPEED * Constants.TILE_SIZE
		var move_distance := pixels_per_second * delta
		var remaining_distance := position.distance_to(_target_position)
