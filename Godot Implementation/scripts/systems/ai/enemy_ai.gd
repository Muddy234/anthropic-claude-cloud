...
func _load_tier_config():
	...
	memory_duration = data.memory_duration if data.has("memory_duration") else 0.0

	# Combat
	windup_duration = data.windup_duration if data.has("windup_duration") else 0.3
	optimal_range = data.preferred_range if data.has("preferred_range") else 1.0
	can_kite = data.kites_behavior if data.has("kites_behavior") else false

	# Social
	pack_courage = data.pack_courage if data.has("pack_courage") else false
	pack_courage_threshold = data.pack_courage_threshold if data.has("pack_courage_threshold") else 4
	is_sacrificial = data.is_sacrificial if data.has("is_sacrificial") else false
	can_sacrifice_minions = data.can_sacrifice_minions if data.has("can_sacrifice_minions") else false
	sacrifice_threshold = data.sacrifice_threshold if data.has("sacrifice_threshold") else 0.3

	# Behavior
	search_behavior = data.search_behavior if data.has("search_behavior") else "none"
	territory_size = data.territory_size if data.has("territory_size") else 20
...
func _state_transitions(can_see: bool, dist: float, is_low_hp: bool, can_flee: bool, hp_pct: float,
	...
	match current_state:
		...
		S.CHASING:
			if can_kite and is_ranged and can_see and dist <= optimal_range + range_tolerance + 1:
...
func _perform_attack():
	...
	var result := GameManager.combat_system.perform_attack(enemy, target)
