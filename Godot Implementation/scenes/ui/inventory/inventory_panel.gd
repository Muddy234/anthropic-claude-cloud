...
@onready var equipment_slots: VBoxContainer = $PanelContainer/HBoxContainer/EquipmentSection/EquipmentSlots
@onready var inventory_grid: GridContainer = $PanelContainer/HBoxContainer/InventorySection/ScrollContainer/InventoryGrid
@onready var gold_label: Label = $PanelContainer/HBoxContainer/InventorySection/GoldLabel
@onready var weight_label: Label = $PanelContainer/HBoxContainer/InventorySection/WeightLabel
...
func _refresh_inventory():
	...
	for i in inventory_slot_refs.size():
		...
		if i < player.inventory.size() and player.inventory[i]:
			var inv_item = player.inventory[i]
			slot.set_item(inv_item.item, inv_item.quantity)
		else:
			slot.clear()
...
func _calculate_weight() -> int:
	...
	var total := 0
	for inv_item in GameManager.player.inventory:
		if inv_item and inv_item.item:
			total += inv_item.item.weight * inv_item.quantity
	return total
...
func _try_equip(from_slot: InventorySlot, equip_slot: InventorySlot):
	...
	if old_item:
		...
		player.remove_from_inventory(from_slot.slot_index)
...
func _try_unequip_to_slot(equip_slot: InventorySlot, inv_slot: InventorySlot):
	...
	if inv_slot.item and equip_slot.accepts_item(inv_slot.item):
		...
		player.inventory[inv_slot.slot_index] = {"item": old_equip, "quantity": 1}
	else:
		player.unequip_item(equip_slot.slot_type)
		if inv_slot.item:
...
func _try_use_item(slot: InventorySlot):
	...
	if item.item_type == Constants.ItemType.CONSUMABLE:
		item_used.emit(item)
		GameManager.player.use_item(slot.slot_index)
		_refresh_inventory()
...
func _get_rarity_color(rarity: Constants.ItemRarity) -> Color:
	match rarity:
		Constants.ItemRarity.COMMON:
			return Color.WHITE
		Constants.ItemRarity.UNCOMMON:
			return Color.GREEN
		Constants.ItemRarity.RARE:
			return Color.BLUE
		Constants.ItemRarity.EPIC:
			return Color.PURPLE
		Constants.ItemRarity.LEGENDARY:
			return Color.ORANGE
		_:
			return Color.WHITE
