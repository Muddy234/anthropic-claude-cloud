...
func _place_stairs(stair_type: Constants.TileType) -> Vector2i:
	if rooms.is_empty():
		# No rooms - find any floor tile
		var fallback := _find_floor_tile_anywhere()
		if fallback != Vector2i(-1, -1):
			tiles[fallback.x][fallback.y] = stair_type
			return fallback
		push_error("No valid tile for stairs placement!")
		return Vector2i(int(width / 2), int(height / 2))

	# Pick a random room
	var room := rooms[randi() % rooms.size()]

	# Find a valid floor tile in the room (away from edges)
	for _attempt in 20:
	...
	# Fallback: search entire room for any floor tile
	for x in range(room.position.x, room.end.x):
		for y in range(room.position.y, room.end.y):
			if x >= 0 and x < width and y >= 0 and y < height:
				if tiles[x][y] == Constants.TileType.FLOOR:
	...
	# Last resort: search any room for a floor tile
	for other_room in rooms:
		for x in range(other_room.position.x, other_room.end.x):
			for y in range(other_room.position.y, other_room.end.y):
				if x >= 0 and x < width and y >= 0 and y < height:
					if tiles[x][y] == Constants.TileType.FLOOR:
						tiles[x][y] = stair_type
						return Vector2i(x, y)

	push_error("Could not place stairs - no floor tiles found!")
	return Vector2i(int(width / 2), int(height / 2))

## Find any floor tile on the map
func _find_floor_tile_anywhere() -> Vector2i:
	for x in width:
		for y in height:
			if tiles[x][y] == Constants.TileType.FLOOR:
				return Vector2i(x, y)
...
func _get_player_spawn(stairs_up: Vector2i) -> Vector2i:
	# First try: spawn near stairs up (check all 8 directions)
	for dir in Constants.ALL_DIRECTIONS:
		var pos: Vector2i = stairs_up + dir
		if is_walkable(pos):
			return pos

	# Second try: if stairs_up itself is walkable, use it
	if is_walkable(stairs_up):
		return stairs_up

	# Third try: find any walkable tile in the room containing stairs
	var room := _get_room_at(stairs_up)
	...
	# Fourth try: find any walkable tile on the map
	return _find_any_walkable_tile()

## Find any walkable tile on the map (emergency fallback)
func _find_any_walkable_tile() -> Vector2i:
	# Search rooms first (more likely to have valid tiles)
	for room in rooms:
		for x in range(room.position.x, room.end.x):
			for y in range(room.position.y, room.end.y):
				var pos := Vector2i(x, y)
	...
	# Search entire map as last resort
	for x in width:
		for y in height:
			var pos := Vector2i(x, y)
			if is_walkable(pos):
	...
	push_error("No walkable tile found for player spawn!")
	return Vector2i(int(width / 2), int(height / 2))
