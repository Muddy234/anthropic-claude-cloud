...
func _place_building(building: Dictionary):
	...
	for dy in building.height:
		for dx in building.width:
			...
			var is_edge: bool = dx == 0 or dx == building.width - 1 or dy == 0 or dy == building.height - 1
			var is_entrance: bool = dx == int(building.width / 2) and dy == building.height - 1
			...
			match building.type:
				...
				"entrance":
					var is_entrance_gap: bool = dx == int(building.width / 2) and dy == building.height - 1
...
func _add_paths():
	...
	var center_x: int = int(town_square.x + town_square.width / 2)
	var center_y: int = int(town_square.y + town_square.height / 2)
...
func _create_path(x1: int, y1: int, x2: int, y2: int):
	var dx: int = 1 if x2 > x1 else -1
	var dy: int = 1 if y2 > y1 else -1
...
func _add_decorations():
	# Add trees
	for i in 15:
		var x: int = 2 + randi() % (WIDTH - 4)
		var y: int = 2 + randi() % (HEIGHT - 4)
	...
	for b in buildings:
		if b.id == "town_square":
			var well_x: int = int(b.x + b.width / 2)
			var well_y: int = int(b.y + b.height / 2)
	...
	for building in buildings:
		if building.type == "building":
			var bench_x: int = building.entrance_x + 1
			var bench_y: int = building.entrance_y + 1
...
func _crush_building(building: Dictionary):
	...
	var center_x: int = int(building.x + building.width / 2)
	var center_y: int = int(building.y + building.height / 2)
